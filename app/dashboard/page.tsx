...
export default async function DashboardPage() {
  ...
  return (
    <><Header /><main className="mx-auto max-w-6xl px-4 py-10 sm:px-6"><div className="flex flex-wrap items-end justify-between gap-4"><div><p className="text-xs font-semibold uppercase text-accent">Workspace</p><h1 className="mt-2 font-serif text-3xl text-heading">{profile.display_name ? `${profile.display_name}'s dashboard` : "Your dashboard"}</h1></div><Link href="/create" className="bg-accent px-5 py-2.5 text-sm font-semibold text-accent-fg">Create video</Link></div>
      ...
      <ConnectedAccountsPanel canConnect={profile.role === "admin"} />
      <div className="mt-8 grid gap-6 lg:grid-cols-2"><section><h2 className="font-serif text-xl text-heading">Recent videos</h2><div className="mt-3 space-y-px bg-edge">{videos?.length ? videos.map((video) => { const signedUrl = videoDownloads.get(video.id); return <article key={video.id} className="bg-surface p-4">{signedUrl && <video controls preload="metadata" playsInline className="aspect-video w-full bg-black" src={signedUrl}>Your browser does not support inline video playback.</video>}<div className="mt-3 flex flex-wrap items-start justify-between gap-3"><div className="min-w-0"><p className="truncate font-medium text-heading">{video.title}</p><p className="mt-1 truncate text-xs text-muted">{video.filename}</p></div><div className="flex shrink-0 items-center gap-3"><time className="text-xs text-muted">{new Date(video.created_at).toLocaleDateString()}</time>{signedUrl && <a href={signedUrl} download={video.filename} className="flex items-center gap-1.5 text-xs font-medium text-accent hover:underline"><Download className="h-4 w-4" aria-hidden="true" />Download</a>}</div></div></article>; }) : <p className="bg-surface p-5 text-sm text-muted">Completed videos will appear here.</p>}</div></section>
