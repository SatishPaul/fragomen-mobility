...
| Social OAuth completion | Signed snapshot validation, tenant-bound callback identity, collision rejection, reused-account reactivation, and pending-profile finalization |
...
* OAuth sends the signed-in user as Outstand's tenant identifier and assigns only the exact callback or finalized account returned under that tenant.
* A reused account can be reactivated only for its existing user. An account assigned to another user cannot be claimed.
...
7. Connect each approved network used by the account. If several profiles or organization pages are offered, select only the approved entries.
8. After OAuth, refresh Dashboard and verify platform, nickname or username, and Ready or Reconnect status. Repeat one connection and confirm the existing assignment is reactivated without a duplicate.
9. Confirm each connected account appears only for its assigned user and administrators.
10. Render a video, select several approved destinations, and verify Recommended labels match the chosen aspect ratio.
11. Stop at the final confirmation unless a real external post is approved.
...
* The full local regression gate passes 13 test files and 81 tests, including focused playback and social-connection route coverage.
